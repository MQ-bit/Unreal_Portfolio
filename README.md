...
입력에서 Ability 활성화, Gameplay Tag 상태 변경, Attribute 반영, UI 갱신등을 코드 단위로 설계하는 데 초점을 맞췄습니다.  
웨이브 서바이벌 모드를 별도로 구성해, 전투 시스템 게임 모드와 스폰 로직, 비동기 로딩, UI 흐름까지 연결될 수 있도록 구현했습니다.
