...
void UWarriorAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetCurrentHealthAttribute())
	{
		...
		PawnUIComponent->OnCurrentHealthChanged.Broadcast(MaxHealthValue > 0.f ? GetCurrentHealth() / MaxHealthValue : 0.f);
	}

	if (Data.EvaluatedData.Attribute == GetCurrentRageAttribute())
	{
		const float NewCurrentRage = FMath::Clamp(GetCurrentRage(), 0.f, GetMaxRage());
		const float MaxRageForUI = GetMaxRage();
		...
		{
			HeroUIComponent->OnCurrentRageChanged.Broadcast(MaxRageForUI > 0.f ? GetCurrentRage() / MaxRageForUI : 0.f);
		}
	}
	...
	if (Data.EvaluatedData.Attribute == GetDamageTakenAttribute())
	{
		...
		const float NewCurrentHealth = FMath::Clamp(OldHealth - DamageDone, 0.f, MaxHealthValue);
