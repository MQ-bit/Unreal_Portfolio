...
void AWarriorSurvialGameMode::Tick(float DeltaTime)
{
    ...
    if (CurrentSurvialGameModeState == EWarriorSurvialGameModeState::WaitSpawnNewWave)
        ...
        if (CurrentSurvialGameModeState == EWarriorSurvialGameModeState::SpawningNewWave)
        {
            ...
            if (TimePassedSinceStart >= SpawnEnemiesDelayTime && AreCurrentWaveEnemiesPreloaded())
            {
		...
		if (CurrentSurvialGameModeState == EWarriorSurvialGameModeState::InProgress
			&& CurrentSpawnedEnemiesCounter == 0
			&& ShouldKeepSpawnEnemies()
			&& AreCurrentWaveEnemiesPreloaded())
		{
			CurrentSpawnedEnemiesCounter += TrySpawnWaveEnemies();
		}
...
{	
	...
	for (const FWarriorEnemyWaveSpawnerInfo& SpawnerInfo : GetCurrentWaveSpawnerTableRow()->EnemyWaveSpawnerDefinitions)
	{
		...
		UClass* const* FoundEnemyClass = PreLoadedEnemyClassMap.Find(SpawnerInfo.SoftEnemyClassToSpawn);
		if (!FoundEnemyClass || !(*FoundEnemyClass))
		{
			continue;
		}

		UClass* LoadedEnemyClass = *FoundEnemyClass;
...
{
	CurrentSpawnedEnemiesCounter = FMath::Max(CurrentSpawnedEnemiesCounter - 1, 0);
