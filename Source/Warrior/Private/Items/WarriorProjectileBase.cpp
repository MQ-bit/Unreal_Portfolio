...
void AWarriorProjectileBase::OnProjectileHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (!OtherActor || OtherActor == this || OtherActor == InstigatorPawn)
	{
		Destroy();
		return;
	}
	...
	if (!InstigatorPawn || !HitPawn || !UWarriorFunctionLibrary::IsTargetPawnHostile(InstigatorPawn, HitPawn))
	{
		Destroy();
		return;
	}
...
{
	if (!InHitPawn || !ProjectileDamageEffectSpecHandle.IsValid())
	{
		return;
	}
